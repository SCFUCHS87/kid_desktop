...
static const char *up_vol[] = {
    "/bin/sh", "-c",
    "amixer set Master 5%+ && notify-send \"Volume Level\" \"$(amixer -D default -M get Master | awk 'NR==6 {print $5}' | sed -e 's/\\[//g' -e 's/\\]//g')\"",
    NULL
};
...
static const char *mute_vol[] = {
    "/bin/sh", "-c",
    "amixer set Master toggle && notify-send \"Volume Level\" \"$(amixer -D default -M get Master | awk 'NR==6 {print $5}' | sed -e 's/\\[//g' -e 's/\\]//g')\"",
    NULL
};

static const char *brighter[] = {
    "/bin/sh", "-c",
    "brightnessctl set 10%+ && notify-send \"Screen Brightness\" \"$(brightnessctl intel_backlight | awk 'NR==2 {print $4}' | sed -e 's/\\[//g' -e 's/\\]//g')\"",
    NULL
};
static const char *dimmer[]   = {
    "/bin/sh", "-c",
    "brightnessctl set 10%- && notify-send \"Screen Brightness\" \"$(brightnessctl intel_backlight | awk 'NR==2 {print $4}' | sed -e 's/\\[//g' -e 's/\\]//g')\"",
    NULL
};
